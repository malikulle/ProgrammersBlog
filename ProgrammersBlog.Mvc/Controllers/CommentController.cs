    ...
    public class CommentController : Controller
    {
        ...
        public async Task<JsonResult> Add(CommentAddDto commentAddDto)
        {
            if (ModelState.IsValid)
            {
                ...
                {                    
                    var commentAjaxViewModel = JsonSerializer.Serialize(new CommentAddAjaxViewModel()
                    {
                        CommentDto = result.Data,
                        CommentAddPartial = await this.RenderViewToStringAsync("_CommentAddPartial", commentAddDto)
                    }, new JsonSerializerOptions() { ReferenceHandler = ReferenceHandler.Preserve });
                    return Json(commentAjaxViewModel);
                }
                ModelState.AddModelError("", result.Message);
            }
            var commentAddAjaxErroModel = JsonSerializer.Serialize(new CommentAddAjaxViewModel()
            {
                CommentAddPartial = await this.RenderViewToStringAsync("_CommentAddPartial", commentAddDto),
                CommentAddDto = commentAddDto
            }, new JsonSerializerOptions() { ReferenceHandler = ReferenceHandler.Preserve });
            return Json(commentAddAjaxErroModel);
        }
    }
}
